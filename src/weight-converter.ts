...
import { WeightUnits } from "./interfaces/weight-units.js";
import { inputs } from "./inputs.js";
import { convertButton, resetButton } from "./button.js";
import { calculation } from "./calculation.js";

class WeightConverter {
    selectTags: NodeListOf<HTMLSelectElement>;
    data: WeightUnits;
    fromWeightValue: string;
    toWeightValue: string;

    constructor(data: WeightUnits, selector: string) {
    ...
    selectWeightUnit(id: string, value: string) {
        if (id === 'weight-from') {
    ...
    convertWeight() {
        ...
        if (!fromWeightValue || !toWeightValue) {
            ...
            if (inputs.fromInput.value.length === 0) {
                ...
                this.displayResult(result as number);
            }
        }
    }
}
